defmodule Phoenix.Tracker.State do
  ...
  defp observe_removes(
         %State{pids: pids, values: values, delta: delta} = local,
         remote,
         remote_map
       ) do
    ...
    foldl(values, init, ms, fn {{topic, pid, key} = values_key, _, tag} = el,
                               {clouds, delta, leaves} ->
  ...
  def merge_deltas(
    ...
    if (Clock.dominates_or_equal?(remote_end, local_start) and
      ...
      {:ok,
       %State{local | clouds: clouds, values: Map.new(all_vals), range: {new_start, new_end}}}
    else
  ...
  def replica_up(%State{replicas: replicas, context: ctx} = state, replica) do
    {%State{
       ...
       | context: Map.put_new(ctx, replica, 0),
         replicas: Map.put(replicas, replica, :up)
  ...
  def replica_down(%State{replicas: replicas} = state, replica) do
    {%State{state | replicas: Map.put(replicas, replica, :down)}, [],
     replica_users(state, replica)}
  end
  ...
  defp put_context(%State{delta: delta, replica: rep} = state, new_ctx) do
    ...
    %State{state | context: new_ctx, delta: %State{delta | range: {start_clock, new_end}}}
  end
