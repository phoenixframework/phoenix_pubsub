defmodule Phoenix.Tracker.StateTest do
  ...
  test "joins are observed via other node", config do
    ...
    {c, [{{_, _, :alice}, _, _}], []} = State.merge(c, State.extract(a, c.replica, c.context))
  ...
  test "removes are observed via other node", config do
    ...
    {c, [{{_, _, :bob}, _, _}], []} = State.merge(c, State.extract(b, c.replica, c.context))

    a = State.leave(a, bob, "lobby", :bob)
    {b, [], [{{_, _, :bob}, _, _}]} = State.merge(b, State.extract(a, b.replica, b.context))
  ...
  test "get_by_pid", config do
    ...
    assert [{{"topic", ^pid, "key1"}, %{}, {{:node1, 1}, 1}}] = State.get_by_pid(state, pid)
  ...
  test "get_by_topic", config do
    ...
    assert [{"key1", %{}}, {"key2", %{}}, {"user3", %{}}] = State.get_by_topic(state, "topic")
