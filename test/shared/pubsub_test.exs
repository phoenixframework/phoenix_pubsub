defmodule Phoenix.PubSubTest do
  ...
  setup config do
    ...
    adapter = Application.get_env(:phoenix_pubsub, :test_adapter)
    start_supervised!({Phoenix.PubSub, adapter: adapter, name: config.test, pool_size: size})
    {:ok, %{pubsub: config.test, topic: to_string(config.test), pool_size: size}}
  end
  ...
  for size <- [1, 8] do
    ...
    test "pool #{size}: broadcast/3 and broadcast!/3 publishes message to each subscriber",
      ...
      :ok = PubSub.broadcast(config.test, config.topic, :ping)
      assert_receive :ping
      :ok = PubSub.broadcast!(config.test, config.topic, :ping)
      assert_receive :ping
    end
    ...
    test "pool #{size}: broadcast/3 does not publish message to other topic subscribers",
         config do
      PubSub.subscribe(config.test, "unknown")

      Enum.each(0..10, fn _ ->
        rpc(spawn_pid(), fn -> PubSub.subscribe(config.test, config.topic) end)
      end)
    ...
    test "pool #{size}: broadcast_from/4 and broadcast_from!/4 skips sender", config do
      ...
      PubSub.broadcast_from(config.test, self(), config.topic, :ping)
      refute_received :ping
    ...
    test "pool #{size}: local_broadcast sends to the current node", config do
      ...
      PubSub.local_broadcast(config.test, config.topic, :ping)
      assert_receive :ping
    end
    ...
    test "pool #{size}: local_broadcast_from/5 skips sender", config do
      ...
      PubSub.local_broadcast_from(config.test, self(), config.topic, :ping)
      refute_received :ping
    end

    @tag pool_size: size
    test "pool #{size}: with custom dispatching", %{topic: topic, test: test} do
